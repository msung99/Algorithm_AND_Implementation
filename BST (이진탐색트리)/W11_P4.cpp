...
public:
	...
	void max_num(int k);  // 이진탐색트리에서 k번째로 큰 정수를 출력 
...
// BST 원소들 중에 k번째로 큰 숫자 출력
void BST::max_num(int k)
{
	sort(key_list.begin(), key_list.end(), greater<int>()); // 오름차순 정렬후

	cout << key_list[k - 1] << endl; // k번째로 큰 숫자 출력
}
...
int main(void)
{
	...
	while (t--)
	{
		...
		else if (a == "height")
		{
			...
			cout << b1.get_height(sub_root) -1 << endl;  // 1 을 A 값을 결과로 출력해야 함에 주의하자!!!
		}
	}
}
