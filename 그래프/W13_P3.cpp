...
// 정점을 저장하는 시퀀스 
// ( 이론 수업떄 배운 것과 달리, 엣지 오브젝트들을 가리키는 시퀀스와, vertex 오브젝트들을 가키리는 시퀀스가 따로 존재하지 않는다.)
class vertexList {
private:
	vertex* header;
	vertex* trailer;
...
// 이 함수는 엣지 오브젝트를 딱 삭제하고 끝날때 활용될 수도 있곘지만, vertex 오브젝트를 삭제하는 연산에서 활용된다!
...
void graph::SumAdjacentNode(int vertexId)
{
	...
	for (int i = 0; i < edgeMatrix.size(); i++)
	{
		for (int j = 0; j < edgeMatrix.size(); j++) 
		{
			if (edgeMatrix[i][j] != NULL && edgeMatrix[j][i] != NULL) // 해당 셀이 어떤 엣지 오브젝트를 가리키고 있는 경우 
			{
				...
				{
					// 이제 엣지 오브젝트가 가리키는 두 정점 오브젝트 src, dst 중에서 vertex 가 아닌 정점에 대해서 더하기 연산을 진행하면 된다.
	...
	// 주의!!!!! => 위에서 더하기 연산을 진행할때 i행 j열에 대해서 더하기를 진행했다면, j행 i열에 대해서도 더하기를 또 다시 진행한다.
	// 따라서 sum 결과를 나누기 2 를 해준다!

	if (sum == 0) // 정점 vertexId 와 연결된 정점이 존재하지 않는 경우
	{  //  => 연결된 정점이 존재하지 않았다면, 더하기 연산을 단 한번도 진행하지 않아서 max 값이 초기에 설정해준 값인 0 일 것이다.
		cout << "None" << endl;
		return;
	}
